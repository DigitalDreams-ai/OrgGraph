...
async function main(): Promise<void> {
  const tempDir = mkdtempSync(join(tmpdir(), 'orgumented-github-context-'));
...
`.trim(),
  ...
  const githubServer = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && url.pathname === '/repos/acme/repo-one/branches') {
        ...
        JSON.stringify([
          {
            ...
            commit: {
              ...
              url: 'https://api.github.test/repos/acme/repo-one/commits/abcdef1234567890'
            }
          },
          {
            name: 'feature/github-context',
            protected: false,
            commit: {
              sha: '1234567890abcdef',
              url: 'https://api.github.test/repos/acme/repo-one/commits/1234567890abcdef'
            }
          }
        ])
      );
    ...
    if (req.method === 'GET' && url.pathname === '/repos/acme/repo-one/pulls') {
      ...
      res.end(
        JSON.stringify([
          {
            number: 17,
            title: 'Add repo context support',
            state: 'open',
  ...
  try {
    ...
    try {
      ...
      const contextResponse = await fetch(`http://127.0.0.1:${apiAddress.port}/github/repo/context`);
      assert.equal(contextResponse.status, 200);
      const contextJson = (await contextResponse.json()) as {
        repo: { fullName: string; defaultBranch: string };
        branches: Array<{ name: string; protected: boolean; lastCommitSha?: string }>;
        pullRequests: Array<{ number: number; title: string; headRef?: string; baseRef?: string; author?: string }>;
      };

      assert.equal(contextJson.repo.fullName, 'acme/repo-one');
      assert.equal(contextJson.repo.defaultBranch, 'main');
      assert.deepEqual(
        contextJson.branches.map((branch) => ({
          name: branch.name,
          protected: branch.protected,
          lastCommitSha: branch.lastCommitSha
        })),
        [
          { name: 'main', protected: true, lastCommitSha: 'abcdef1234567890' },
          { name: 'feature/github-context', protected: false, lastCommitSha: '1234567890abcdef' }
        ]
      );
      assert.deepEqual(
        contextJson.pullRequests.map((pullRequest) => ({
