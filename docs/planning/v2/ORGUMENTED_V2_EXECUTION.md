...
- wave6 semantic-frame shadow mode now also covers the bounded review approval family: approval-style review asks emit replay-safe `approval_decision` frames with grounded targets, while latest-retrieve approval review asks emit blocked `evidence_scope_unsupported` shadow frames to mirror the current fail-closed behavior
