...
- Ask latest-retrieve prompt generation now also covers Connected App, Permission Set Group, and Custom Permission members, keeping deterministic component-usage lookup aligned with more of the metadata families the runtime already parses
...
- wave6 evidence-lookup now also accepts Connected App, Permission Set Group, and Custom Permission family-qualified prompts, keeping deterministic metadata-component lookup aligned with the existing ingestion/runtime family surface instead of stopping at the earlier UI-oriented subset
- wave6 evidence-lookup now preserves folder-qualified Email Template fullNames and `.../email/<folder>/<template>.email-meta.xml` file-path forms, so Ask and latest-retrieve prompts keep deterministic component-usage targeting for real Salesforce email template metadata instead of collapsing to basename-only lookup
...
- wave7 follow-up now also renders packet grounding spotlights directly in Ask: review packets expose top automation names plus top impacted sources, and retrieved flow packets expose top citation source files as first-class packet sections instead of hiding those cues inside free-form text or follow-up actions
- wave7 evidence-lookup follow-up now renders metadata-component usage asks as dedicated packets (`Usage signals`, `References`, `Coverage`, `Component family`) with source-spotlight references and Browser/Refresh follow-up actions, so component-usage answers no longer fall back to generic approval-card framing
...
- wave10 follow-up now also hard-wraps long Ask follow-up action labels and path-heavy rationales with explicit `packet-action-*` guards, so dense decision packets stay readable without horizontal spill even when operator guidance includes long diagnostic paths
- wave10 follow-up now also wraps metadata-component usage packet target labels and definition anchors with explicit `path-value` guards, and extends `test:ask-guards` so long metadata-arg component names and reference source paths stay bounded in Ask
...
- wave10 follow-up now also locks the Org Browser limited-coverage warning path with explicit browser render assertions and wrapping guards, so long discovery warnings and coverage instructions stay bounded when the live family catalog is partial or truncated
- wave12 operator-safety follow-up now carries per-family catalog origin through the Browser stack, keeps fallback-only family rows visible for context, and blocks those rows from driving retrieve in both UI and API so limited-coverage real-org sessions no longer depend on operator guesswork
- wave10 follow-up now also locks Org Sessions long-string rendering for selected alias, username, org ID, and instance URL values, and adds a dedicated `test:connect-render` regression gate so the connect surface stays bounded under real-org alias/session identifiers
...
- wave11 follow-up now runs the phase17 human benchmark publication regression in `validate`, so canonical benchmark publication cannot drift from local-only tooling behavior
...
- Settings & Diagnostics telemetry cards now expose card-local quick actions (`Refresh Ask Trust`, `Open Runtime Telemetry`, `Refresh Runtime Telemetry`, `Refresh Status`) so trust and runtime follow-up does not depend on returning to the shared top action row
...
- wave8 telemetry follow-up now also exposes `/metrics` inside Settings & Diagnostics as a structured runtime telemetry card (request volume, hottest route, slowest route, latest route, recent non-200 failure signatures), reducing the need to inspect raw metrics payloads for route timing and failure correlation
- wave8 diagnostics follow-up now lifts Ask trust and runtime telemetry into the top structured operator triage snapshot, so replay coverage and route-failure signals are visible with direct actions before the operator scrolls into secondary diagnostics cards
- wave8 diagnostics follow-up now also exposes a structured Meta Adapt delta summary (relation changes, sample delta, formula-version change, added/removed/updated relations) so adaptation drift no longer requires raw before/after payload inspection
- wave8 analyze follow-up now gives system-permission edge states a deterministic mapping-recovery path in the structured snapshot: unresolved grant context tells the operator to run `Diagnose User Mapping` first and withholds premature Ask handoff until mapping is resolved
- wave8 diagnostics follow-up now also gives Meta Context and Meta Adapt their own card-local actions, so semantic weighting refresh and deterministic adapt reruns do not depend on returning to the shared top action row
- wave8 diagnostics simplification now treats the structured triage snapshot and the primary top controls as the actionable surfaces, while Ask trust, runtime telemetry, and meta-adapt detail cards become secondary read-only evidence panes instead of repeating their own quick-action rows
- wave8 diagnostics follow-up now also removes the separate runtime and tooling checklist cards under the structured snapshot, so recovery text is shown once in the primary triage surface instead of as duplicated lower-card prose
...
- overall implementation is still early, roughly 65%
...
- typed changed-file scope is now live in the engine and Connect workspace for:
